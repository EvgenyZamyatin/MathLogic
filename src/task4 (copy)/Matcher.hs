...
data Error = NoError | Error1 String Term | Error2 Term Exp | Error3 String Term  Exp | SomeError deriving (Show, Eq)
...
canSubsitude binded (Var x) t = 
	foldl 
	(\start -> \x -> if (start /= NoError) then start else (if (S.member x binded) then Error1 x t else NoError)) 
	NoError (varList t)
